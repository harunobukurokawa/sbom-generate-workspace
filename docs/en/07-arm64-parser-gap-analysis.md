# 07. ARM64 SBOM Parser Gap Analysis and Remediation
...
### 4.4 Correcting the expectations the experiment started from

The tree-sitter work was begun on the strength of the following figures. They were
**expectations written before implementation, never measurements**, and this PoC
contradicts them. They are recorded here so the claim does not resurface.

| Originally claimed | Measured |
|--------------------|----------|
...
### 4.5 Disposition of tree-sitter-bash (incomplete; kept out of this branch)

Beyond being a regression when registered, measurement showed the implementation
itself is **incomplete**:

| Capability | Status |
|------------|--------|
| AST construction / control-flow extraction | ✓ works (`then_body` / `else_body` are recovered) |
| I/O file extraction | ✗ **not implemented** — `extractIOFiles()` always returns empty lists |
| Python wrapper JSON handling | ✗ **broken** — the Node script emits two pretty-printed JSON blocks, but the wrapper calls `json.loads()` on line 1 only, which fails and silently falls back to regex |

Consequently `ParseResult.inputs` / `outputs` are always empty and five of its tests
fail.

Because none of it is used, **the experimental code is not part of this branch.** It
is preserved on the `experiment/tree-sitter-bash` branch (Node.js parser, Python
wrapper, KernelSbom integration shim, its tests, `package.json`, and the original
proposal document with a correction prepended). Keeping it here would add ~1,150
unused lines and an npm dependency to review for no benefit.
...
- SBOM generation on this branch needs **no Node.js and no npm packages**.
...
1. Added `_parse_flask_codegen()` and registered it with a narrow pattern
   (`.*xsm/flask/policy/mk(flask|access_vector)\.sh\b`). **This is the only parser the
   arm64 gap actually required.**
2. Added `_validate_obj_tree()` plus the total-drop warning in `_keep_existing()`, so
   the failure mode described in section 2 reports itself instead of looking like a
   parser defect.
3. Documented the "keep these patterns narrow" constraint on `XEN_COMMAND_PARSERS`.

Two parsers were written during the investigation and then **rejected on measurement**
(section 4.2), so neither appears here: an `ld` parser (already upstream, and
over-matching) and the tree-sitter registry entry (zero rescues, seven steals).

Tests (`scripts/xen-sbom-poc/tests/test_xen_parsers.py`, 21 passing):
...
- `TestKeepExistingWarnsOnTotalDrop` (4) — the total-drop warning fires, and stays
  silent on partial drops, on empty input, and when `OBJ_TREE` is unset.
- `TestObjTreeValidation` (4) — the upfront check is silent for a configured build
  directory, detects the repository-root mistake and names the directory to use
  instead, warns when there is no `.config` anywhere, and is silent when `OBJ_TREE`
  is unset.
- The remaining 9 are the pre-existing tests for the compat/binfile/combine/mv
  parsers and the injection itself, unchanged by this work.
...
4. **Look at config differences, not architecture differences.** The gap was
   ...
   (backlog B-10).
5. **Never document expectations as measurements.** The "99.6%" that justified
   adopting tree-sitter was never measured, and measurement refuted the hypothesis
   behind it outright (section 4.4).
