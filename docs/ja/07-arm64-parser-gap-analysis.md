...
tree-sitter の作業は以下の数値を根拠に始まった。これらは**実装前に書かれた期待値で
あり測定値ではなく**、本 PoC の測定で否定された。主張が再燃しないよう記録しておく。
...
### 4.5 tree-sitter-bash の扱い（未完成・本ブランチには含めない）

レジストリに登録すると退行になるうえ、実装自体が**未完成**であることを測定で確認した。

| 機能 | 状態 |
|------|------|
| AST 構築・制御フロー抽出 | ✓ 動作（`then_body` / `else_body` を復元できる） |
| I/O ファイル抽出 | ✗ **未実装**。`extractIOFiles()` が常に空配列を返す |
| Python ラッパーの JSON 分離 | ✗ **バグ**。Node 側が pretty-print された JSON を 2 ブロック出力するが、ラッパーは 1 行目のみを `json.loads()` するため失敗し、無警告で正規表現フォールバックに落ちる |

結果として `ParseResult.inputs` / `outputs` は常に空であり、テスト 5 件が失敗する。

一切使用していないため、**実験コードは本ブランチに含めない。**
`experiment/tree-sitter-bash` ブランチに保存してある（Node.js パーサー、Python
ラッパー、KernelSbom 統合シム、そのテスト、`package.json`、および訂正を冒頭に
追記した元の提案文書）。本ブランチに残すと、未使用の約 1,150 行と npm 依存を
レビュー対象に加えることになり、見返りが無い。

- **完成は critical path ではない。** tree-sitter なしで未知コマンド 0 件を
  達成しているため、投資対効果が現時点では無い。
- 本ブランチの SBOM 生成は **Node.js も npm パッケージも不要**。
...
1. `_parse_flask_codegen()` を追加し、`XEN_COMMAND_PARSERS` に登録（狭いパターン
   `.*xsm/flask/policy/mk(flask|access_vector)\.sh\b`）。**arm64 の欠落が実際に
   必要としたのはこの 1 個だけ。**
2. `_validate_obj_tree()` と `_keep_existing()` の全件削除警告を追加。2 節で述べた
   失敗モードが、パーサー欠陥に見えるのではなく自ら名乗るようにした。
3. `XEN_COMMAND_PARSERS` に「パターンを狭く保つ」旨のコメントを追記。

調査中に書いたが**実測により却下した**パーサーが 2 個あり（4.2 節）、いずれも
ここには含まれない: `ld` パーサー（上流に既存かつ過剰マッチ）と tree-sitter の
レジストリ登録（救済 0 件・奪取 7 件）。

テスト（`scripts/xen-sbom-poc/tests/test_xen_parsers.py`, 計 21 件パス）:
...
- `TestKeepExistingWarnsOnTotalDrop`（4 件）— 全件削除で警告が出ること、および
  部分削除・空入力・`OBJ_TREE` 未設定では警告しないこと。
- `TestObjTreeValidation`（4 件）— 設定済みビルドディレクトリでは無警告、
  リポジトリ root の誤りを検出して代わりに渡すべきディレクトリを名指しすること、
  `.config` がどこにも無い場合の警告、`OBJ_TREE` 未設定時の無警告。
- 残る 9 件は compat/binfile/combine/mv パーサーと注入機構の既存テストで、
  本作業による変更はない。
...
4. **アーキテクチャ差ではなくコンフィグ差を見る。** 欠落の実体は「ARM64」ではなく
   ...
   （バックログ B-10）。
5. **期待値を実測値として文書化しない。** tree-sitter 導入の根拠だった「99.6%」は
   一度も測定されておらず、実測は仮説そのものを否定した（4.4 節）。
