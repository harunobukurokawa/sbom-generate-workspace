...
未知コマンド警告は**わずか6件**（*誤り。実際は 317 件 — 本節末尾の訂正を参照*）。
その他（`gcc`, `ld`, `objcopy`, `nm`, `ar`, `strip`）は
...
> **訂正（2026-08-06）:** 上記のうち2つの数値が誤りである。以降の設計がこれらを前提に
> 書かれているため、本文はそのまま残す。
>
> 1. **「未知コマンド警告はわずか6件」**、および系統ごとの「4件」「2件」という内訳は、
>    採取済みの実行ログから再現できない。`analysis/xen-poc/xen-poc.run.log` にある
>    KernelSbom の警告サマリは、3件の例示コマンドに続けて
>    `... (Found 314 more instances of this warning)` を出力している。すなわち
>    スキップされたコマンドは **317件**、加えて未対応の `IfBlock` が1件である。
>    上記に挙げた3つのコマンド*系統*自体は正しいが、件数は正しくない。
> 2. **「3つのパーサエントリ追加で完全へ到達する」**は作業量を過小評価していた。
>    `fail-on-unknown` は最初の未解析コマンドで停止するため、パーサを1つ追加する
>    たびにグラフが1層深く降り、次の Xen 固有レシピが露出した。未知コマンド 0 の
>    達成には `compat-*` 系統、`combine_two_binaries.py`、`binfile`、素の `cat`、
>    `.banner` の no-op に加え、Xen の「`X.new` を生成してから `mv`」イディオムに
>    対応する存在フィルタが必要で、合計約200行を要した。詳細は
>    `docs/{en,ja}/04-xen-parsers-implementation.md` と `worklog/journal.md`。
