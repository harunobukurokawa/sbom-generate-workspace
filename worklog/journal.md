...
> **訂正（2026-08-06）:** 上記6項と残作業に書いた `strace` ベースの方式は、
> 翌日のコミット `1b2ffbd` で置き換えられている（当時 journal を更新し忘れた）。
> `execve` のみのトレースでは各コマンドの作業ディレクトリを argv が絶対パスを
> 含む場合（実測で約18%）しか復元できず、信頼できなかったため。現行スクリプトは
> make の `SHELL` を `bash -x` に差し替え、`PS4='+++XENSBOM_CWD:$PWD+++ '` で
> 全コマンドの直前に作業ディレクトリを出力させる方式で、`-j1` 実行。出力も
> `xen-tools-build.strace.log` ではなく `xen-tools-build.trace.log` と
> `xen-tools-build.stdout.log` の2本。経緯はスクリプト冒頭のコメントに記載。なお調査時に作成した一時ディレクトリ
...
## 2026-08-06 B-6（arm ビルドでの検証）— arm64 で未知コマンド 0 件

Docker 環境（Ubuntu 22.04, 12コア）で arm64 版 SBOM 生成を実施した。結果は
**未知コマンド 0 件 / 895 パス / 1,951 elements / 1.4 MB**。KernelSbom は無改造を維持。
詳細は `docs/{ja,en}/07-arm64-parser-gap-analysis.md`、手順は手順書 8 節。

### 誤診とその原因（本作業の主要な学び）

arm64 ビルド自体は成功したのに、生成 SBOM が **7 elements・追跡ファイル 1 件**
（`prelink.o` 自身のみ）しか無く、当初これを「複雑なシェル構文がパースできていない」と
誤診した。tree-sitter パーサーの登録、`ld` パーサーの自作追加などを試したが改善せず。

エラーメッセージのパス（`/workspace/xen/common/built_in.o`）が実体
（`/workspace/xen/xen/common/built_in.o`）と 1 階層ずれていることに気づき原因を特定:
**`--obj-tree` にリポジトリ root を渡していた**（正しくはハイパーバイザ dir）。
`.cmd` 内のパスはビルドディレクトリ相対のため全入力が非存在パスに解決され、
`_keep_existing()` が**無警告で全件削除**していた。パーサーは最初から正しく動いていた。

対策として 2 段構えの検出を入れた（`xen_parsers.py` 内で完結、KernelSbom 無改造）:
1. **事前検証** `_validate_obj_tree()` — `<OBJ_TREE>/.config` が無く 1 階層下に有れば
   「`pass <OBJ_TREE>/xen instead`」と具体的に提案。ビルド 1 回分を無駄にする前に検出。
2. **事後警告** 非空の入力が全件削除された際に警告。実装前に頻度を計測し、健全な
   ビルドでは 292 回中 1 回のみ（`.banner.tmp`）、誤設定時は約 290 回で判別力十分と確認。

### 真の欠落は XSM/FLASK コード生成 2 件のみ

obj-tree を是正すると未知コマンドは 2 種に絞られた（`mkflask.sh` /
`mkaccess_vector.sh`）。x86 に無かった理由は **`arm64_defconfig` が XSM/FLASK を
有効化するから**であり、arch 差ではなくコンフィグ差（ADR-0011）。バックログ B-6 の
「arch 依存差の洗い出し」という前提自体が的を外していた。

### tree-sitter-bash は実測により不採用（ADR-0010）

全 303 savedcmd で定量評価したところ、追加した 2 パーサーは両方とも退行だった:
- `_parse_ld_command`: 救済 0 / 奪取 23。上流に `^([^\s]+-)?ld\b` が既存。かつ
  `.*ld\b` が「`build` を含む任意コマンド」に過剰マッチ（`gcc -Ibuild/include ...`）。
- tree-sitter パーサー: 救済 0 / 奪取 7。`_VALIDATION_PRELUDE` と `IfBlock` 処理が
  対象構文を先に吸収済みで寄与余地なし。

導入根拠だった「48% → 99.6%」は一度も測定されておらず、実装前の期待値だった。
実験コードは未完成（`extractIOFiles()` が常に空を返す）でもあり、
`experiment/tree-sitter-bash` ブランチに分離して本流には入れない。

### 作業上の反省

1. **grep の断片で結論を出しかけた。** 健全ビルドのログで、別警告（上流の
   `Could not infer primary purpose`）の集計行「Found 7 more instances」を新警告の
   ものと誤認し「8 件出ている」と一度判断した。文脈ごと確認して誤りと判明（実際 1 件）。
2. **未検証の主張を書いた。** 「99.6%」を実測値として書いた誤りを訂正した同じ文書内で、
   今度は「tree-sitter は単体では動作する」を未検証で書いていた。実行したらテスト
   5 件が失敗した。動作主張は書く前に実行して確かめる。
3. **fetch せずにブランチを作った（最も影響が大きい）。** PR 用ブランチをローカルの
   古い `main`（`45832c9`）から作成したが、origin/main は既に 6 コミット進んでいた。
   結果、英語版手順書を二重に作成し（所有者が `82f86ab` で既に作成済み）、
   文書番号 `06`・ADR 番号 `0005/0006`・バックログ `B-8` を衝突させた。
   下記のとおり作り直した。**ブランチ作成前に必ず fetch する。**

### PR 用ブランチの構成（衝突解消後）

`feature/arm64-sbom-support`（`origin/main` = `3e2f3f0` 起点）:
- コード 2 コミット（`xen_parsers.py` / `test_xen_parsers.py` のみ。所有者が触って
  いないため衝突なし）
- 文書 1 コミット。衝突解消の内容:
  - 英語版手順書は**自作を破棄し所有者版を採用**。arm64 節と 1.3 の実測時間、
    フロー図、節 11 の限界記述だけを追加した（節 8 を挿入し 8〜11 を 9〜12 に繰り下げ）
  - arm64 分析文書を `06-` → **`07-`**（`06-external-validation.md` が使用中）
  - ADR を **0010 / 0011**（所有者が 0009 まで使用済み）
  - 新規バックログを **B-9 / B-10 / B-11**（所有者の B-8 はトレーサビリティ照会で別物）
  - 所有者の B-6 を ✅ に更新
- あわせて、両手順書の節 11 に残っていた「外部 SPDX バリデータでの検証は未実施
  （B-1）」を削除した。所有者が B-1 を完了し `06-external-validation.md` を
  作成済みで、記述が事実と矛盾していたため。

tree-sitter 実験の経緯（Node.js 12 互換性修正、未完成の詳細など）は
`experiment/tree-sitter-bash` ブランチ側の記録に残してある。

## 2026-08-06 リポジトリ整合性監査（コミットログと実体の突き合わせ）

「commit log に記載されている更新内容に沿ったファイル構成・内容になっているか」
という依頼を受け、全 20 コミットと作業ツリーを突き合わせた。

**まず作業ツリーが未解決マージで停止していた。** `origin/feature/arm64-sbom-support`
を pull した際の衝突が放置され、6 ファイルにコンフリクトマーカーが残存。うち
`xen_parsers.py` にも 9 個入っており、本体コードが Python として壊れた状態だった。

原因はブランチの二重運用。リモート側は merge-base `45832c9` から切られており、
その後ローカルに積んだ 6 コミット（B-1 外部検証、B-3 採取スクリプト、prelink.o
説明図、`sbom_analysis` 訂正、B-8 仕様）を持たないまま、arm64 の作業だけを
独自に 3 コミットで実施していた。つまり同じ変更が二系統で別々に実装されていた。

差分を全て確認したうえで、ローカルがリモートの上位互換であると判断し、全衝突を
`--ours` で解決（マージ後ツリー = マージ前 HEAD）。根拠:
- `xen_parsers.py` の差分は doc リンクの `06-`/`07-` 表記 3 行のみ、テストは同一。
- リモート版の英語 runbook には「外部 SPDX 検証は未実施（B-1）」という、
  ローカルでは既に削除済みの古い記述が残っていた。
- リモートは `06-external-validation.md` を持たないため arm64 文書を `06-` と
  採番していた。取り込むと番号衝突かつ `07-` との 99% 重複になるので破棄。

**次に、コミットメッセージと実体の矛盾を 4 件修正した。**

1. `39cee54` の 06→07 採番修正がテストファイルだけ漏れており、
   `test_xen_parsers.py` の docstring が `docs/*/06`（= 外部検証文書）を指していた。
2. README と `docs/{en,ja}/03` の「未知コマンド警告はわずか 6 件」「3 つのパーサで
   完全になる」が実ログと不一致。`analysis/xen-poc/xen-poc.run.log` の警告サマリは
   `... (Found 314 more instances of this warning)` を出しており実際は 317 件。
   後者は journal 側では既に訂正済みだったが README が追随していなかった。
   03 は Phase A 成果物で以降の設計がこの数値の上に書かれているため、`779a333` に
   倣い本文を残して訂正ブロックを追加。README は現況要約なので直接書き換え。
   併せて README の「9 unit tests」→ 21、両 runbook の「残作業: 外部バリデータ検証」
   （B-1 完了済み）も修正。
3. B-3 の記録が `1b2ffbd` の方式変更（strace → `bash -x` + PS4）に追随しておらず、
   journal は依然 strace コマンドを、backlog は strace/bear を書いていた。
   journal は時系列記録なので訂正注記を追加、backlog は現況リストなので書き換え。
4. README / CLAUDE.md が 7 月時点で凍結されていた。README は docs 一覧が 01〜03
   のみ、Status が「Phase A 完了、次フェーズで 3 パーサ + tools/libs + Safety Case」
   と Phase B 完了済みの記述と自己矛盾。CLAUDE.md は「外部 SPDX 検証には `@context`
   の展開が必要（B-1 未完了）」と、事実と正反対の記述を残していた。両方を現況に更新し、
   arm64 手順・`validate-spdx.sh`・`run-xen-tools-build.sh`・XSM/FLASK パーサ・
   obj-tree 自己診断を追記。

**所見**: 実データに基づく記述（ADR-0008/0009 の関係数、1,519/1,442/3,554 等）は
再計算で全て一致し、精度は高い。崩れていたのは要約層（README/CLAUDE.md）と
作業ログの追随、およびブランチ運用の 3 点だった。

**後日談（同日）**: 上記のマージコミット `ff22d33` は、その後 rebase で落として履歴を
linear 化した。push 直前に `git ls-remote` でリモートの実状態を確認したところ、
マージ相手だった `eb51b36` 系統（`e298d36` / `ccbf158` / `eb51b36` の 3 コミット）は
リモートのどのブランチからも到達不可能な孤立状態になっていた（`experiment/tree-sitter-bash`
からも到達不可）。ユーザーによれば tree-sitter-bash ブランチを削除しようとした際の
副作用らしい。マージコミットを残すと、その孤立系統がマージ親としてリモート履歴に
復活してしまうため、`git rebase --onto 39cee54 ff22d33` でマージごと落とした。
衝突解決が全て `--ours` だったのでツリーは不変（tree hash `841e424` で一致を確認）。

なお `experiment/tree-sitter-bash`（`20a0f36`）は**削除しないこと**。tree-sitter 実験の
経緯はこのブランチ側にしか残っておらず、本ジャーナルの前段からも参照している。
...
## 2026-08-20 伊藤さんのレビュー指摘を反映（スライド3点修正・B-12 起票）

FuSa SIG 向け状況報告スライド（日英各版、`analysis/presentations/2026-08-19-fusa-sig-status/`）
について、伊藤さんから3点のレビュー指摘を受け、各点の妥当性と
「Linux共通かXen固有か」を検証した上で反映した。

1. **Page 3 ③（`prelink.o` を起点に SPDX を導出）への前提の補足**: 指摘は妥当。
   ただし検証の結果、「ビルドへの組み入れ（ファイル単位）を追跡すれば十分で、
   関数呼び出し関係までは追わない」という前提は **Xen 固有ではなく KernelSbom
   自体の設計方針**であり、Linux カーネルの SBOM でも同じであることを
   `docs/ja/01-linux-sbom-tool.md`（KernelSbom は入力→出力のファイル/ビルド
   単位の関係のみを扱う）で確認した。Xen 固有なのは「Xen のビルド環境に
   即したパーサーが別途必要だった」という部分（P3②・P5で既出）であり、
   両者を混同しないよう、P3 に1文の注釈ボックスを追加するに留めた
   （伊藤さん提案通り、Appendix化はしていない）。
2. **Page 6（トレーサビリティ照会ツールの「必要性」）の FuSa への言及**:
   指摘は妥当。現在の文言「FuSa の変更管理プロセスが必要とする問い」は、
   `worklog/backlog.md` の B-0（未着手）が「Xen FuSa SIG が SBOM/SPDX を
   実際に必要としているか自体が未確認」と明記している内容と矛盾していた
   ため、断定を弱め「変更影響分析」という一般的な表現に修正した。
   代替案として提示された VEX（脆弱性情報）との関連づけは、リポジトリ内に
   一切検討記録がなく同程度に未検証の主張だったため、そのまま採用はせず、
   「将来的に活用できる可能性がある（検証は未着手）」と明示的にヘッジした
   表現でユーザーの同意を得て追加した。
3. **残課題への「パーサー層の保守方針」追加**: 指摘・判断ともに妥当。ただし
   ユーザーからは「OSS である以上ビルド定義が変化するのは Linux でも同じで
   あり、あまり強調しすぎる必要はない」との補足があったため、深刻な欠陥では
   なく「Xen コミュニティとして今後どう保守体制を構築するかが未確定であり、
   Xen Summit 等での短い議論が望ましい」という中立的なトーン（優先度は
   「要協議」、赤色などの警告色は使わない）で新規バックログ項目
   **B-12** として起票した。Linux では `scripts/sbom/` が Linux ツリー内に
   あり保守がLinux本流の開発体制に組み込まれているのに対し、現状の
   `xen_parsers.py` は本プロジェクト側の外部レイヤーに留まっている、という
   構造的な違いを明記した。

3点とも日英スライド（`xen-sbom-status-report.pptx` / `-en.pptx`）に反映し、
スライドは10枚構成（本編10枚+Appendix区切り+Appendix A）に更新。
`worklog/backlog.md` に B-12 を追加。
