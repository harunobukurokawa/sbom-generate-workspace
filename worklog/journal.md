...
## 2026-08-06 B-9（無警告削除の検出）と B-8（手順書への arm64 反映）

### B-9: `_keep_existing()` の無警告全件削除に警告を追加

設計判断の前に、健全な arm64 ビルドでの発生頻度を計測した（`_keep_existing` を
計装）。結果: 292 回の呼び出しのうち全件削除は **1 回のみ**（`.banner.tmp` =
正当な一時ファイル）。誤った obj-tree では約 290 回発生するため、判別力は十分と
判断して実装した。

実装は 2 段構え:
1. **事前検証** `_validate_obj_tree()` を `install_xen_extensions()` から呼ぶ。
   OBJ_TREE に `.config` が無く 1 階層下（`<OBJ_TREE>/xen/.config`）に有る場合、
   「リポジトリ root を渡している」と判定して `pass <OBJ_TREE>/xen instead` と
   具体的に提案する。ビルド 1 回分を無駄にする前に気づける。
2. **事後警告** `_keep_existing()` が非空の入力を全件削除した際に警告。パス一覧と
   obj-tree を含めて actionable にした。`sbom_logging` は同一テンプレートを集約
   するため、健全時は 1 行・異常時は「Found N more instances」となり件数自体が
   診断情報になる。

検証: 誤った obj-tree（リポジトリ root）で実行すると事前警告が出ることを確認。
健全ビルドでは新警告 1 件のみで SBOM は 1,951 elements のまま。テスト 9 件追加
（全件削除で警告 / 部分削除で無警告 / 空入力で無警告 / OBJ_TREE 未設定で無警告 /
事前検証 4 パターン）。計 23 passed, 10 skipped。

**途中のミス**: 健全ビルドのログを grep した際、別警告
（上流の `Could not infer primary purpose`）の集計行「Found 7 more instances」を
新警告のものと誤認し、「警告が 8 件出ている」と一度判断した。ログを文脈ごと
確認して誤りと判明（新警告は 1 件）。grep の断片で結論を出さないこと。

### B-8: 再現手順書への arm64 反映

`docs/ja/05-reproduction-runbook.md` に「手順6: arm64 版での再現（クロスビルド）」を
追加（8 節として挿入、後続 8〜11 節を 9〜12 に繰り下げ）。あわせて旧番号体系のまま
残っていた相互参照 3 箇所（手順6→3、手順6→4、手順7→5）と「上記10節」→「上記11節」を修正。

内容は既存書式（コマンド → 期待される結果の実績値）に合わせ、特に以下を明記:
- **第 2 引数はハイパーバイザ dir（`external/xen/xen`）でリポジトリ root ではない**
  ことを最重要注意として枠囲みで記載。B-9 の警告文も併記
- ビルド後に `make clean` しないこと（ポストビルド SBOM のため中間 .o が必要）
- 想定内の警告 2 種とその件数（1 件なら正常、多数なら階層ずれを疑う）
- テスト期待値を「全9件 pass」から「23 passed, 10 skipped」に更新。skip は
  tree-sitter の未完成分で意図的、failed が 1 件でもあれば再現失敗と明記

**実地検証**: 手順書の記述どおり（相対パス `external/xen/xen`、引数もそのまま）に
リモートで実行し、期待値と照合した。elements 1,951 / software_File 894 /
Relationship 758 / build_Build 290 / 拡張子内訳すべて一致。

照合中に `used-files` の件数が 894 と 895 で揺れる原因を特定した:
ファイル末尾に改行が無いため `wc -l` は 894 と出るが、実際のパスは 895 個。
差の 1 個はルート成果物 `prelink.o`（`sbom-output.spdx.json` 側にある）。
また `../../../usr/bin/dash`（`/bin/sh` の実体）が含まれる — コード生成を実行した
シェル自身が依存として追跡されたもので異常ではない（上流 binfile パーサー由来）。
この精度を手順書と `docs/{ja,en}/06` の両方に反映した。

## 2026-08-06 B-7: 再現手順書の英語版を作成（bilingual 同期を回復）

`docs/en/05-reproduction-runbook.md` を新規作成。これで CLAUDE.md の bilingual
同期規約における非同期は解消（`01`〜`06` すべて ja/en 揃った）。

### 翻訳前に日本語版の古い記述を修正

B-7 の前提は「日本語版が実機追試で確定していること」。読み直したところ、11節
（既知の限界）に **「arm/arm64 での検証は未実施（backlog B-6）。本書の手順・数値は
すべて x86_64」** という記述が残っていた。B-6 完了・8節で arm64 を扱っている以上
これは虚偽なので、先に修正した:
- arm64 は検証済み、ただし **arm32 は未検証**と限定
- 他 defconfig の網羅性は未確認（B-10）という項目を追加。arm64 の知見から
  「欠落は有効化された機能に依存する」と分かったため

あわせて 2節の全体フロー図に手順6を追加し、「手順6 は手順1 だけを前提とし
手順2〜5 と独立に実行できる」ことを明記した。

### 所要時間を実測して追加

1.3 の所要時間表は x86_64 のみだったので arm64 を実測して追加。推測値を書かない
ため実際に計測した:
- **arm64 ビルド 8.0 秒**（12コア、`-j12`）
- **arm64 SBOM 生成 0.8 秒**（303 個の `.cmd` を解析）

最初の計測で「ビルド 9 秒」と出たとき速すぎると疑い、`make clean` が本当に効いて
いるかを検証した（clean 後に `prelink.o` なし・`.cmd` 0 個・`built_in.o` 0 個を確認）。
効いていたので値は正しく、ハイパーバイザー本体は規模が小さいだけだった。
表にもこの検証条件を明記した。

### 同期の機械的検証

- 見出し数 20 = 20、節番号の並びが全 20 箇所で一致、コードブロック数 26 = 26
- 主要な数値 30 種を両版で grep して出現回数が全一致することを確認
  （1,951 / 894 / 895 / 303 / 624 / 13,796 / 15,282 / 8.0 / 0.8 など）
- 英語版に `docs/ja` 参照が残っていないこと、日本語版に `docs/en` が混入して
  いないことを確認

### 英語版の記載どおりに実機実行して照合

8.3 の生成コマンド、8.3 の確認コマンド（`wc -l` = 894、flask/policy 5 件）、
10 節の検証スクリプト（1,951 elements・context が SPDX 3.0.1）、9 節のテスト
（23 passed, 10 skipped）をすべて英語版の記述そのままで実行し一致を確認した。
