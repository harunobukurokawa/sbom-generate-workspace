...
### B-12 ⬜ P?（要 Xen コミュニティ合意）— パーサー層の保守方針
- 背景: 伊藤さんのレビュー指摘（2026-08-20、FuSa SIG 向け資料のレビュー時）。
  `xen_parsers.py` は Xen のビルド定義（Makefile 等）の変更に追従する必要があるが、
  現状は警告ログの目視確認や SBOM の充足度チェックによる手動運用の試作段階に
  留まっている。
- 注記（強調しすぎない）: ビルド定義が OSS として変化していくこと自体は Linux
  カーネルでも同様であり、殊更に強調すべき欠陥ではない。ただし Linux では
  パーサー（`DEFAULT_COMMAND_PARSER_REGISTRY`）の保守が Linux 本流の開発体制
  （`scripts/sbom/` が Linux ツリー内にあり、Kbuild に変更を入れる開発者が
  パーサーの要否を意識できる立場にある）に組み込まれているのに対し、現状の
  Xen 拡張（`xen_parsers.py`）は本プロジェクト側の外部レイヤーに留まっており、
  Xen 本体の開発フローには組み込まれていない点が異なる。
- Xen としてこの保守体制をどう構築するか（本プロジェクトが持ち続けるか、B-4 の
  CI 統合を機に Xen 本体へ寄せるか等）は未確定。**Xen Summit 等でのコミュニティ
  との短い議論が望ましい**、という位置づけに留め、技術的な優先度は付けない。
- 完了条件: 保守方針についてXenコミュニティとの合意形成の方向性を得る、または
  継続討議事項として明記する。
