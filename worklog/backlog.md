...
### B-8 ⬜ P2 — arm64 検証結果の再現手順書への反映
- `docs/ja/05-reproduction-runbook.md` は x86 前提。arm64 の手順（クロスツールチェーン、
  `arm64_defconfig`、`--obj-tree` は**ハイパーバイザ dir** を指す点）を追記する。
- `--obj-tree` の階層ずれは実際に誤診を招いた（B-9 参照）ので、明示的な注意書きを入れる。
- 完了条件: 手順書に arm64 節が追加され、追試で再現できること。B-7（英語版）と同期。

### B-9 ⬜ P2 — `_keep_existing()` の無警告全件削除に警告を出す
...
- 対応案: 非空の入力集合が空になった場合に `sbom_logging.warning` を出す。
  `xen_parsers.py` 内の変更で完結（KernelSbom 無改造を維持）。
- 完了条件: 誤った obj-tree で実行した際に警告が出ることをテストで固定。
...
## 推奨順序（2026-08-06 改訂）
**B-9 → B-8 → B-1 → B-10 → B-3 → B-0 →（B-0=Yes なら）B-2 → B-4 → B-7 → B-5 → B-11**。
- B-6 は完了（arm64 検証）。その過程で B-8/B-9/B-10/B-11 が派生した。
- **B-9 を最優先**にする。小さな変更で済むうえ、これが無いと同じ誤診（obj-tree の
  階層ずれをパーサー欠陥と誤認）を将来また繰り返す。今回の主要な時間損失要因。
- B-8 は B-6 の成果を再現可能にするための締め。B-7（英語版）は B-8 の後に同期。
