...
**B-7 → B-1 → B-10 → B-3 → B-0 →（B-0=Yes なら）B-2 → B-4 → B-5 → B-11**

- 2026-08-06 に **B-6 → B-9 → B-8** を完了。B-6（arm64 検証）の過程で
  B-8/B-9/B-10/B-11 が派生し、うち B-8/B-9 を同日消化した。
- **次は B-7**（手順書の英語版）。日本語版が arm64 を含む形で実機追試済みになった
  ので、翻訳の前提（CLAUDE.md の bilingual 同期規約）が整った。`docs/{ja,en}/06` は
  既に bilingual なので、残る非同期は `05` のみ。
- B-1（外部バリデータ）は Phase B の締め。B-10 は B-5（上流貢献）の説得材料になる
  （「arch 差ではなくコンフィグ差」という知見は上流にとっても価値がある）。
